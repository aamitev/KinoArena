...
public class Cinema {
	...
	public Cinema(String name, String email, String gsm, String address) throws ModelException{
		setName(name);
		setEmail(email);
		setGSM(gsm);
		setAddress(address);
	...
	private void setEmail(String email) throws ModelException {
		...
		}else throw new ModelException(ILLEGAL_EMAIL);
	}

	private void setName(String name) throws ModelException {
		if(Utils.checkString(name)) {
			this.name = name;
		}else throw new ModelException(INVALID_CINEMA_NAME);
	}
